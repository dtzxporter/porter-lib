...
pub struct IKHandle {
    ...
    pub target_offset: Option<Vector3>,
...
impl IKHandle {
    ...
    pub const fn new(name: Option<String>, start_bone: usize, end_bone: usize) -> Self {
        Self {
            name,
            start_bone,
            end_bone,
    ...
    pub const fn target_bone(mut self, bone: usize) -> Self {
        self.target_bone = Some(bone);
        self
    }

    /// Sets the target offset.
    pub const fn target_offset(mut self, offset: Vector3) -> Self {
        self.target_offset = Some(offset);
        self
    }

    /// Sets the pole vector bone index.
    pub const fn pole_vector_bone(mut self, bone: usize) -> Self {
        self.pole_vector_bone = Some(bone);
        self
    }

    /// Sets the pole bone index.
    pub const fn pole_bone(mut self, bone: usize) -> Self {
        self.pole_bone = Some(bone);
        self
    }

    /// Sets whether or not to use the target bone's rotation.
    pub const fn use_target_rotation(mut self, value: bool) -> Self {
