...
impl Bone {
    ...
    pub const fn segment_scale_compensate(mut self, ssc: bool) -> Self {
    ...
    pub fn local_matrix(&self) -> Matrix4x4 {
        Matrix4x4::create_trs(self.local_position, self.local_rotation, self.local_scale)
    }

    /// Gets the world matrix (T * R * S).
    pub fn world_matrix(&self) -> Matrix4x4 {
        Matrix4x4::create_trs(self.world_position, self.world_rotation, self.world_scale)
    }
}
